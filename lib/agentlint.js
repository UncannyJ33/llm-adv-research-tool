...
// The complete set of tools an evidence-bounded role may declare. An allowlist, because
// CLAUDE.md: "Screening is fail-open; verification is fail-closed" — and the verifier is
// verification. The previous denylist (/web|search|fetch|browser|crawl|http/) claimed to be
// caught-by-default and was the opposite: `tools: Bash, Read, Task` lints clean under it,
// and Task dispatches a subagent that can search. `Agent`, `Skill` and every `mcp__*` name
// slipped through the same way. Anything not on this list is a violation whether or not
// anyone has yet realised what it can reach.
const EVIDENCE_BOUNDED_TOOLS = ['Bash', 'Read'];
...
function lintAgents({ agentsDir, skillsDir } = {}) {
  ...
  for (const file of files) {
    ...
    if (EVIDENCE_BOUNDED.includes(name)) {
      const disallowed = splitTools(fm.tools).filter(t => !EVIDENCE_BOUNDED_TOOLS.includes(t));
      if (disallowed.length) {
        add(name, 'evidence-boundary',
          `evidence-bounded role declares ${disallowed.join(', ')} — may declare only `
          + `${EVIDENCE_BOUNDED_TOOLS.join(', ')}, because its only input is `
...
module.exports = {
  lintAgents, parseFrontmatter, splitTools, EVIDENCE_BOUNDED, EVIDENCE_BOUNDED_TOOLS,
};
