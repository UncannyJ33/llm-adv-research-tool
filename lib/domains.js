...
const WEB_CLASS_RULES = [
  ...
  // happening inside it is not. Both patterns stay anchored to the forge host AND to a real
  // route shape, so a journal volume at /issues/12 cannot match and neither can a repo
  // directory that happens to be named `issues`.
  //   1. flat forges: github.com/owner/repo/issues, codeberg.org/owner/repo/pulls
  //   2. GitLab, which nests subgroups arbitrarily deep and always separates the project
  //      path from the route with `/-/`. Requiring exactly owner/repo missed every project
  //      in a subgroup — ordinary on GitLab — and put the thread back at official-source.
  { class: 'repo-discussion', patterns: [
    /(?:github|gitlab|codeberg)\.[a-z.]+\/[^/]+\/[^/]+\/(?:issues|pull|pulls|merge_requests|discussions)(?:[/?#]|$)/i,
    /gitlab\.[a-z.]+\/(?:[^/]+\/){2,}-\/(?:issues|pull|pulls|merge_requests|discussions)(?:[/?#]|$)/i,
  ] },
  ...
  // inflation, pointed the other way. Only the raw-content host: matching
  // `githubusercontent.com` by bare host also promoted user-images. and avatars., so a
  // screenshot pasted into a bug report outranked the paper it was disputing.
  { class: 'source-repo', patterns: [/github\.com/i, /raw\.githubusercontent\.com/i, /gitlab\.com/i, /codeberg\.org/i, /git\.kernel\.org/i] },
