...
function toMarkdown({ state, corpus, ledger, brief = null, topics = [] }) {
  ...
  const section = (title, claims) => {
    ...
    for (const c of claims) {
      ...
      if (last) {
        s += `  - Reason: ${last.reason}\n`;
        if (last.override_reason) s += `  - Override: ${last.override_reason}\n`;
        if (last.role_warning) s += `  - Warning: ${last.role_warning}\n`;
      }
    }
    return s;
  };
  ...
  body += section('Contested claims — verifiers disagreed', ledger.contested());
