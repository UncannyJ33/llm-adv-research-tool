...
function writeFileAtomic(file, text) {
  ...
  try {
    ...
    // The write failed, so this temp file holds a partial state no reader wants. `list`
    // filters on run.json and would ignore it, but nothing else ever deletes one: a writer
    // that keeps failing strands a fresh temp file per attempt, each named with a random
    // suffix, until the run directory is full of them.
