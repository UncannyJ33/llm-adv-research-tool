...
function claimBlocks(claims) {
  ...
  for (const c of claims) {
    ...
    if (last) {
      html += `<div class="rej-line"><span class="k">Reason</span> ${escapeHtml(last.reason)}</div>`;
      if (last.override_reason) {
        html += `<div class="rej-line override"><span class="k">Override</span> ${escapeHtml(last.override_reason)}</div>`;
      }
      html += `<div class="rej-line"><span class="k">Span</span> `
        + `${last.quoted_span ? `&ldquo;${escapeHtml(last.quoted_span)}&rdquo;` : 'no supporting span found'}`
        + ` <span class="basis">${escapeHtml(last.span_check)}`
        + `${last.span_role ? ` / ${escapeHtml(last.span_role)}` : ''}`
        + ` / ${escapeHtml(last.evidence_basis)}</span></div>`;
      if (last.reason_quality === 'low') {
      ...
      if (last.role_warning) {
        html += `<div class="rej-line flag">${escapeHtml(last.role_warning)}</div>`;
      }
    }
    html += '</div>';
  }
